...
function getTaskHint(type: string) {
  const hints: Record<string, string> = {
    single_choice: "Прочитайте вопрос, выберите один вариант ответа и нажмите кнопку проверки. После ответа появится результат и пояснение.",
    matching: "Сопоставьте элементы из левого и правого списков и нажмите кнопку проверки — вы узнаете, сколько пар собрано верно.",
    ordering: "Расставьте элементы в правильном порядке и нажмите кнопку проверки, чтобы узнать результат.",
    text_input: "Впишите ответ в поле и нажмите кнопку проверки. Регистр букв значения не имеет.",
    visual_novel: "Читайте сцены и выбирайте варианты реплик — от ваших решений зависит развитие истории и финал.",
    hidden_objects: "Внимательно осмотрите изображение и кликните по всем скрытым предметам из списка.",
    who_am_i: "Прочитайте подсказки одну за другой и выберите, о каком персонаже идёт речь.",
    memo: "Переворачивайте карточки по очереди и запоминайте их расположение, чтобы быстрее находить одинаковые пары.",
    assemble_outfit: "Подберите подходящий предмет для каждого элемента образа и нажмите кнопку проверки.",
  };

  return hints[type] ?? "Прочитайте задание и выполните его так, как описано в условии. После завершения появится результат и пояснение.";
}
